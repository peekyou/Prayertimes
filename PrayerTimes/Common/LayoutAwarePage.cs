...
namespace PrayerTimes.Common
{
    ...
    public class LayoutAwarePage : Page
    {
        ...
        {
            string title = loader.GetString("Preferences");
            SettingsCommand cmd = new SettingsCommand(title, title, (x) =>
            {
                OpenSettings();
            });

            string privacy = loader.GetString("PrivacyStatementString");
            SettingsCommand cmdPrivacy = new SettingsCommand(privacy, privacy, (x) =>
            {
                privacyPolicyPopup = new Popup();
                privacyPolicyPopup.Closed += OnPrivacyPolicyClosed;
                Window.Current.Activated += OnWindowActivated;
                privacyPolicyPopup.IsLightDismissEnabled = true;
                privacyPolicyPopup.Width = privacyPolicyWidth;
                privacyPolicyPopup.Height = windowBounds.Height;

                PrivacyPolicy privacyPolicy = new PrivacyPolicy();
                privacyPolicy.Width = privacyPolicyWidth;
                privacyPolicy.Height = windowBounds.Height;

                privacyPolicyPopup.Child = privacyPolicy;
                privacyPolicyPopup.SetValue(Canvas.LeftProperty, windowBounds.Width - privacyPolicyWidth);
                privacyPolicyPopup.SetValue(Canvas.TopProperty, 0);
                privacyPolicyPopup.IsOpen = true;
            });

            args.Request.ApplicationCommands.Clear();
            args.Request.ApplicationCommands.Add(cmd);
            args.Request.ApplicationCommands.Add(cmdPrivacy);
        }
        ...
        protected void OpenSettings()
        {
            ...
            SettingsNarrow mypane = new SettingsNarrow((PrayerViewModel)DataContext);
        ...
        async void OnPopupClosed(object sender, object e)
        {
            await ((SettingsNarrow)settingsPopup.Child).SaveSettings();

            Window.Current.Activated -= OnWindowActivated;
        }

        async void OnPrivacyPolicyClosed(object sender, object e)
        {
            Window.Current.Activated -= OnWindowActivated;
        }
